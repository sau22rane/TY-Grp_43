...
void Read_matrix(char mat_name, LOCAL_MATRIX_T* local_A,GRID_INFO_T* grid, int n);
            ...
            for (grid_col = 0; grid_col < grid->q; grid_col++) {
                ...
                if (dest == 0) {
                    for (mat_col = 0; mat_col < Order(local_A); mat_col++)
                        fscanf(fp, "%f", (local_A->entries)+mat_row*Order(local_A)+mat_col);
                } else {
                    for(mat_col = 0; mat_col < Order(local_A); mat_col++)
                        fscanf(fp, "%f", temp + mat_col);
